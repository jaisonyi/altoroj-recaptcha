...
import com.ibm.security.appscan.altoromutual.util.RecaptchaUtil;
...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Verify the Google reCAPTCHA challenge before doing anything with the credentials.
		String recaptchaResponse = request.getParameter("g-recaptcha-response");
		if (!RecaptchaUtil.verify(recaptchaResponse, request.getRemoteAddr())) {
			Log4AltoroJ.getInstance().logError("Login blocked: reCAPTCHA verification failed for user: " + request.getParameter("uid"));
			session.setAttribute("loginError", "Please confirm that you are not a robot and try again.");
