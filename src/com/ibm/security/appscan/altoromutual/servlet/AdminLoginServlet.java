...
public class AdminLoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Verify the Google reCAPTCHA challenge before checking the admin password.
		String recaptchaResponse = request.getParameter("g-recaptcha-response");
		if (!RecaptchaUtil.verify(recaptchaResponse, request.getRemoteAddr())) {
			request.setAttribute("loginError", "Please confirm that you are not a robot and try again.");
